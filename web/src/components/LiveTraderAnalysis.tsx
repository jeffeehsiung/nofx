...
export function LiveTraderAnalysis({ traderId }: LiveTraderAnalysisProps) {
  ...
  const analysis: FeedbackAnalysis | undefined = data?.data || data // handle envelope or raw
  ...
  if (error) {
    return (
      <div className="py-12 text-center text-sm" style={{ color: '#F6465D' }}>
        {language === 'zh' ? '获取分析失败' : 'Failed to fetch analysis'}
      </div>
    )
  }
    ...
    return (
      <div className="py-12 text-center" style={{ color: '#5E6673' }}>
        ...
        <div className="text-sm">
          {language === 'zh' ? '等待交易积累后再查看' : 'Wait for more live trades to accumulate'}
        </div>
      </div>
    )
  }
