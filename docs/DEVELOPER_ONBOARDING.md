...
1. **Read in order:**
   - main.go (lines 1-170)
   - trader/trader.go (lines 1-300)
   - decision/engine.go (lines 1-200)

2. **Trace one decision:**
   - Set breakpoint in `decision.MakeDecision()`
   - Start system, trigger one trade
   - Step through: prompt → AI → parse → execute
...
3. **Check database:**
   ...
   SELECT * FROM positions WHERE closed_at IS NOT NULL LIMIT 5;
   ```

### Day 2: Understand New Features (2-3 hours)

1. **Read calibration system:**
   - decision/threshold_calibrator.go
   - decision/trade_failure.go
   - docs/threshold-calibration.md

2. **Run tests:**
   ```bash
   cd decision
   go test -v -run TestThresholdCalibrator
   ```

3. **Try manual calibration:**
   ```go
   // In main.go, add test function
...
### Day 3: Run Backtest & Analyze (2-3 hours)

1. **Run a backtest:**
   - Web interface → Backtest tab
   - Select: BTCUSDT, ETHUSDT
   - Period: Last 30 days
...
### Day 4: Verify Integration (2-3 hours)

1. **Run verification script** (section 7.1)

2. **Check for missing integrations:**
   - Is trade outcome being saved?
   - Is failure analysis running?
   - Is microstructure used in decisions?

3. **Fix any gaps** (section 9)

### Day 5: Optimize & Test (2-3 hours)

1. **Pick one failure pattern**

2. **Make hypothesis:**
   - e.g., "Increase volume threshold to reduce false breakouts"
...
### Issue 1: "I don't see trade failure analysis in logs"

**Diagnosis:**
```bash
grep "AnalyzeFailedTrade" trader/*.go backtest/*.go
# If no results → Not integrated yet
```

**Solution:** Follow section 9.1 to integrate

---

### Issue 2: "Calibration says 'insufficient data'"

**Diagnosis:**
```sql
SELECT COUNT(*) FROM trade_outcomes;
-- If < 100: Not enough data collected
```

**Solution:**
```bash
# Option 1: Run more backtests to collect data
# Option 2: Import historical trades
# Option 3: Use default thresholds until you have 100+ trades
```

---

### Issue 3: "Microstructure analysis not affecting decisions"

**Diagnosis:**
```bash
grep "Microstructure" decision/engine.go decision/prompt_builder.go
# Check if microstructure data is included in AI prompt
```

**Solution:**
```go
// In decision/prompt_builder.go
func (pb *PromptBuilder) Build(data MarketData) string {
    // ... existing code
    
    // ADD: Include microstructure in prompt
    prompt += "\n## Market Microstructure:\n"
    prompt += fmt.Sprintf("Support Levels: %v\n", data.Microstructure.SupportLevels)
    prompt += fmt.Sprintf("Resistance Levels: %v\n", data.Microstructure.ResistanceLevels)
    prompt += fmt.Sprintf("Order Book Imbalance: %.2f\n", data.Microstructure.Imbalance)
...
```
Week 1: Architecture & Core Flow
  ├─ Read: main.go, trader.go, decision/engine.go
  ├─ Trace: One complete trade decision
  └─ Output: Understand data flow

Week 2: New Features & Calibration
  ├─ Read: threshold_calibrator.go, trade_failure.go
  ├─ Test: Run calibration tests
  └─ Output: Understand failure analysis

Week 3: Backtest Analysis
  ├─ Run: Multiple backtests
  ├─ Analyze: Identify failure patterns
  └─ Output: Optimization hypotheses

Week 4: Integration & Optimization
  ├─ Verify: All features wired up
...
- [ ] I can explain the data flow from startup to trade execution
- [ ] I can trace how an AI decision is made (prompt → AI → parse → execute)
- [ ] I understand how trade failure analysis works (ROC, Youden's J)
- [ ] I know how to run and analyze a backtest
- [ ] I can identify why a backtest lost money (failure patterns)
- [ ] I verified all new features are integrated and used by default
- [ ] I ran the code audit script and understand unused functions
- [ ] I successfully ran calibration on synthetic data
- [ ] I know where each major component is in the codebase
- [ ] I can make a hypothesis, test it via backtest, and deploy if improved

---

## 🎓 Graduation Exercise

**Task:** Improve your system's Sharpe ratio by 20%

**Steps:**
1. Run baseline backtest (save results)
2. Analyze losses (identify dominant failure reason)
3. Make hypothesis for fix
4. Implement fix
5. Run treatment backtest
6. Compare results
7. If Sharpe improved by 20%+ → You've mastered the system! 🎉

---

**Need Help?**
- 📖 Check existing docs: [docs/README.md](../docs/README.md)
- 🐛 Found a bug? Check: [CIRTICAL_ISSUES.md](../CIRTICAL_ISSUES.md)
- 💬 Join community: See [README.md](../README.md) for links
